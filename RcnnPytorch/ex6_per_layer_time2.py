...
if __name__ == '__main__':
    ...
    axs.bar(np.arange(len(latency_edge)), np.array(latency_edge), label='Edge device')
    axs.bar(np.arange(len(data_transmission)), np.array(data_transmission), bottom=latency_edge,
               label='Data transmission')
    axs.bar(np.arange(len(latency_server)), np.array(latency_server),
               bottom=np.array(latency_edge) + np.array(data_transmission), label='Edge server')
    axs.plot(np.array(throughput), 'k.-', label='Timeslice')
    axs.scatter(best_throughout_point, total_latency + 80, s=70, marker='d', color='r', label='Highest throughout')
    axs.scatter(partition_point, latency[partition_point] + 80, s=70, marker='*', color='gold',
                   label='Optimal partition point')
    plt.savefig('.fig/fig_ex6')
