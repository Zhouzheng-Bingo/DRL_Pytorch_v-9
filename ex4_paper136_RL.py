...
class TaskOffloadingEnv(gym.Env):
    ...
    def compute_reward(self, latency, throughput):
        # Initialize base latency threshold
        base_latency_threshold = 480  # 480ms as your base threshold

        # Dynamic Threshold Adjustment: Linearly decreases from 480ms to a minimum of 100ms
        dynamic_latency_threshold = base_latency_threshold * (1 - 0.015 * self.current_task)
        dynamic_latency_threshold = max(dynamic_latency_threshold, 100)  # Ensuring a minimum threshold
        ...
        # 归一化安全阈值
        normalized_safe_threshold = (dynamic_latency_threshold - min_latency) / (max_latency - min_latency)
        ...
        # 基于超出或未达阈值的比例调整奖励
        if normalized_latency <= normalized_safe_threshold:
            # 奖励增量按未达到阈值的比例给出
            reward_increment = 5 * (normalized_safe_threshold - normalized_latency) / normalized_safe_threshold
            reward += reward_increment
        else:
            # 惩罚按超出阈值的比例增加
            penalty_factor = (normalized_latency - normalized_safe_threshold) / (1 - normalized_safe_threshold)
            reward -= 10 * penalty_factor  # 惩罚值增加
    ...
    def step(self, action):
        ...
        if self.current_task < 25:
            ...
            reward = self.compute_reward(latency, throughput)
