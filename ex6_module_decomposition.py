        ...
        if self.n_calls % self.check_freq == 0:
            ...
            for _ in range(10):  # Evaluate over 10 episodes
                ...
                while not done:
                    ...
                    ep_reward += reward
                    step_rewards.append(reward)  # 添加这一步的奖励到列表
                    episode_actions.append(action)
                episode_rewards.append(ep_reward)
                total_rewards.append(sum(step_rewards))  # 添加总奖励到列表
            ...
            self.logger.record('evaluation/mean_total_rewards', np.mean(total_rewards))  # 记录平均总奖励

            # 记录每个步骤的奖励
            for step_index, reward in enumerate(step_rewards):
                self.logger.record(f'steps/step_reward_{step_index}', reward)
...
if __name__ == '__main__':
    ...
    model.learn(total_timesteps=10000, callback=[eval_callback, custom_callback])
